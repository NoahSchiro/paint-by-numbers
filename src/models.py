...
class Discriminator(nn.Module):
    def __init__(self, size):
        ...
        # in: 3 x size x size
        self.c1 = nn.Conv2d(3, 64, kernel_size=4, stride=2, padding=1, bias=False)
        self.b1 = nn.BatchNorm2d(64)
        # out: size x size/2 x size/2

        self.c2 = nn.Conv2d(64, 128, kernel_size=4, stride=2, padding=1, bias=False)
        self.b2 = nn.BatchNorm2d(128)
        # out: size*2 x size/4 x size/4

        self.c3 = nn.Conv2d(128, 256, kernel_size=4, stride=2, padding=1, bias=False)
        self.b3 = nn.BatchNorm2d(256)
        # out: size*4 x size/8 x size/8

        self.out_size = size // (2**3)

        self.linear = nn.Linear(256 * self.out_size**2, 1)
...
class Generator(nn.Module):
    def __init__(self, latent_vec_size, img_size):
        ...
        self.linear = nn.Linear(self.latent_size, 256*(self.img_size//8)**2)

        # Turn a random vector into an image
        self.ct1 = nn.ConvTranspose2d(256, 128, kernel_size=4, stride=2, padding=1, bias=False)
        self.b1  = nn.BatchNorm2d(128)

        self.ct2 = nn.ConvTranspose2d(128, 64, kernel_size=4, stride=2, padding=1, bias=False)
    ...
    def forward(self, rand_vec):
        ...
        rand_vec = rand_vec.view(rand_vec.size(0), 256, self.img_size // 8, self.img_size // 8)
